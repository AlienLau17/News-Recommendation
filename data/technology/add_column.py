...
with open("2019_04_28_13_22h_technology.csv" , "r") as csvinput:
    r = csv.reader(csvinput)
    with open("2019_04_28_technology.csv", "w") as csvoutput:
        w = csv.writer(csvoutput)
        next(r, None)
        w.writerow(["url", "title", "description", "urlToImage", "date"])
        for row in csv.reader(csvinput):
...
with open("2019_04_30_17_22h_technology.csv" , "r") as csvinput:
    r = csv.reader(csvinput)
    with open("2019_04_30_technology.csv", "w") as csvoutput:
        w = csv.writer(csvoutput)
        next(r, None)
        w.writerow(["url", "title", "description", "urlToImage", "date"])
        for row in csv.reader(csvinput):
